...
section .data
    ...
    menu     dB 10,10," MENU :",10
     ...
     n1msg		dB "  	Enter 1st number    : "
	 n1msg_len EQU $-n1msg 

    n2msg		dB "  	Enter 2nd number    : "
	 n2msg_len EQU $-n2msg 

     rsamsg		dB "  	Equivalante SA no. is   :"
	 rsamsg_len EQU $-rsamsg 

     rsbmsg		dB " 	      Equivalente SHA no. is   :"
	 rsbmsg_len EQU $-rsbmsg 

     exitmsg	dB "		Exit from program...",10
	 exitmsg_len EQU $-exitmsg 

     errmsg		dB "		Invalid choice"
...
SA :
	...
	mov  word[ansh],00		;variable cnt directly initialize to 0 ...so 1st ini bx=0 n then these
	mov  word[ansl],00
...
SHA :
     ...
     xor  ebp,ebp					;initialize to 0
     xor  ebx,ebx

     mov  rcx,16

     mov  ax,[n1]					;tacking n1,n2 in register
     mov bx,[n2]

back11: shl  ebp,1					;multiply *2
       shl  ax,1					;only shift purpose
       jnc  next11
       add  ebp,ebx				;in this method 1st reg for shifting and 2nd for adding   
 							; ebp=ans  eax=1st  ebx=2nd
next11 : dec  rcx
       jnz  back11

       Print rsbmsg,rsbmsg_len
       mov   eax,ebp
       call display_8
